...
const STATS = [
  ...
  { k: 'Đã thu', v: '68%', bar: 68, link: 'Xem theo tuyến', to: '/routes' },
  { k: 'Quá hạn', v: '1.120', sub: 'đối tượng', warn: true, link: 'Xem 1.120 quá hạn', to: '/debts' },
]

const QUICK = [
  { icon: 'add', label: 'Thêm đối tượng', to: '/objects' },
  { icon: 'edit', label: 'Tạo/sửa hợp đồng', to: '/objects' },
  { icon: 'route', label: 'Gán tuyến', to: '/routes' },
  { icon: 'percent', label: 'Đề nghị miễn giảm', to: '/debts' },
]
          ...
          <div className="cb-col">
            <section className="cb-card cb-routes">
              <div className="cb-card-head">
                ...
                <button className="cb-arrow" onClick={() => nav('/routes')}>Gán tuyến ›</button>
              </div>
              {ROUTES.map((r) => {
            ...
            <section className="cb-card cb-todo">
              ...
              <div className="cb-todo-row"><span><b className="warn">2</b> lỗi auto-check</span><button className="cb-mini warn" onClick={() => nav('/invoices')}>Xử lý</button></div>
              <div className="cb-todo-row"><span><b>12</b> dòng treo</span><button className="cb-mini" onClick={() => nav('/invoices')}>Xử lý</button></div>
              <div className="cb-todo-row"><span><b>5</b> đề nghị chờ lãnh đạo</span><button className="cb-mini" onClick={() => nav('/debts')}>Xem</button></div>
            </section>
          </div>
        </div>
      </div>
